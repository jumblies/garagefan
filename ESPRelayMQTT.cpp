...
#define MQTT_SERVER "10.10.10.142"  //you MQTT IP Address
...
const int switchPin1 = BUILTIN_LED;
...
char const* switchTopic1 = "/garage/fan1/";
...
void setup() {
  ...
  ArduinoOTA.setHostname("garagefan"); // A name given to your ESP8266 module when discovering it as a port in ARDUINO IDE
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println(topicStr);
   ...
   if (topicStr == "/garage/fan1/")
    {

     //turn the switch on if the payload is '1' and publish to the MQTT server a confirmation message
     if(payload[0] == '1'){
       digitalWrite(switchPin1, HIGH);
       client.publish("/garage/fan1Confirm/", "1");
       }

      //turn the switch off if the payload is '0' and publish to the MQTT server a confirmation message
     else if (payload[0] == '0'){
       digitalWrite(switchPin1, LOW);
       client.publish("/garage/fan1Confirm/", "0");
       }
     }
